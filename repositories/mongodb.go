...
func NewDBCollections(config *config.Config) *DBCollections {
	...
	// set users index
	keys, err := bson.ParseExtJSONObject(`{ "username": 1}`)
	...
	_, err = userCollection.Indexes().CreateOne(context.Background(), usersIndex)
	if err != nil {
		log.Fatal(err)
	}

	// set roles index
	keys, err = bson.ParseExtJSONObject(`{ "role": 1, "level": 1}`)
	options, err = bson.ParseExtJSONObject(`{ "unique": true }`)
	if err != nil {
		log.Fatal(err)
	}
	...
	_, err = roleCollection.Indexes().CreateOne(context.Background(), rolesIndex)
	if err != nil {
		log.Fatal(err)
	}
